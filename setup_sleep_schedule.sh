...
[Service]
Type=oneshot
ExecStart=/bin/bash -ec 'rtcwake -m disable >/dev/null 2>&1 || true; target=\$(date -d "$WAKE_TIME" +%%s); now=\$(date +%%s); if [ "\$target" -le "\$now" ]; then target=\$(date -d "$WAKE_TIME tomorrow" +%%s); fi; rtcwake -m mem -l -t \$target'
EOF
